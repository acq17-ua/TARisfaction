...
class ejFibonacciAction(object):
    ...
    def execute_cb(self, goal):
        ...
        self._result.secuencia_final = []
        self._result.secuencia_final.append(0)
        self._result.secuencia_final.append(1)
        ...
        # check that preempt has not been requested by the client
            ...
            # media de toda la secuencia hasta ahora, al cuadrado
            #print(sum(self._feedback.secuencia_actual))
            newElem = int(self._feedback.secuencia_actual[i]) + int(self._feedback.secuencia_actual[i-1])
            avg = (sum(self._feedback.secuencia_actual)+newElem)/(len(self._feedback.secuencia_actual)+1)
            self._feedback.secuencia_actual.append(sqrt(avg))

            # llevar la verdadera secuencia
            true_newElem = int(self._result.secuencia_final[i]) + int(self._result.secuencia_final[i-1]) 
            self._result.secuencia_final.append(true_newElem)
